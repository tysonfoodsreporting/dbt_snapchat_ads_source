...
final as (
    ...
    select
        ...
        TIMESTAMP(CAST(created_at AS {{ dbt.type_timestamp() }}), "America/Chicago") as created_at,

        name as campaign_name,
        --cast(_fivetran_synced as {{ dbt.type_timestamp() }}) as _fivetran_synced,
        TIMESTAMP(CAST(_fivetran_synced AS {{ dbt.type_timestamp() }}), "America/Chicago") as _fivetran_synced,
        --cast(updated_at as {{ dbt.type_timestamp() }}) as updated_at,
        TIMESTAMP(CAST(updated_at AS {{ dbt.type_timestamp() }}), "America/Chicago") as updated_at,
        (daily_budget_micro / 1000000.0) as daily_budget,
        --cast(start_time as {{ dbt.type_timestamp() }}) as start_time,
        TIMESTAMP(CAST(start_time AS {{ dbt.type_timestamp() }}), "America/Chicago") as start_time,
        --cast(end_time as {{ dbt.type_timestamp() }}) as end_time,
        TIMESTAMP(CAST(end_time AS {{ dbt.type_timestamp() }}), "America/Chicago") as end_time,
